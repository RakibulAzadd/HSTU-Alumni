...
Class Action {
	...
	function signup(){
		extract($_POST);
		$data = " name = '".$firstname.' '.$middlename."' ";
		$data .= ", username = '$email' ";
		$data .= ", password = '".md5($password)."' ";
	...
	// 	// Prepare user data (name, email, password)
	...
	// 	$save = $this->db->query("INSERT INTO users set ".$data);
	// 	if($save){
	// 		$uid = $this->db->insert_id; // Get the newly inserted user ID
	
	// 		// Prepare alumnus_bio data
	// 		$data = '';
	// 		foreach($_POST as $k => $v){
	// 			if($k == 'password') continue; // Don't save password in alumnus_bio table
	// 			if(empty($data) && !is_numeric($k)){
	// 				$data = " $k = '$v' ";
	// 			} else {
	// 				$data .= ", $k = '$v' ";
	...
	// 			$move_img = move_uploaded_file($_FILES['img']['tmp_name'], 'assets/uploads/' . $img_fname);
	// 			if($move_img){
	// 				$data .= ", avatar = '$img_fname' "; // Save image path
	// 			}
	// 		}
	...
	// 				$pdf_fname = strtotime(date('y-m-d H:i')).'_'.$_FILES['pdf']['name'];
	// 				$move_pdf = move_uploaded_file($_FILES['pdf']['tmp_name'], 'assets/uploads/' . $pdf_fname);
	// 				if($move_pdf){
	// 					$data .= ", pdf_file = '$pdf_fname' "; // Save PDF path
	// 				}
	// 			} else {
	// 				return 3; // Return error if the uploaded file is not a PDF
	// 			}
	// 		}
	...
	// 		$save_alumni = $this->db->query("INSERT INTO alumnus_bio set $data ");
	// 		if($data){
	// 			$aid = $this->db->insert_id; // Get the alumnus_bio ID
	
	// 			// Update users table to link the alumnus_id
	// 			$this->db->query("UPDATE users set alumnus_id = $aid where id = $uid ");
	...
	function save_committee(){
		...
		if($_FILES['img']['tmp_name'] != ''){
						$_FILES['img']['name'] = str_replace(array("(",")"," "), '', $_FILES['img']['name']);
						$fname = strtotime(date('y-m-d H:i')).'_'.$_FILES['img']['name'];
						$move = move_uploaded_file($_FILES['img']['tmp_name'],'assets/uploads/'. $fname);
					$data .= ", img = '$fname' ";
		...
		if(empty($id)){
			...
			$save = $this->db->query("INSERT INTO committee set ".$data);
		}else{
			$save = $this->db->query("UPDATE committee set ".$data." where id=".$id);
		}
		if($save)
			return 1;
	}
	...
	function delete_committee(){
		extract($_POST);
		$delete = $this->db->query("DELETE FROM committee where id = ".$id);
		if($delete){
			return 1;
		}
	}
